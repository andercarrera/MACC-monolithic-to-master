...
class Coordinator(object):
    ...
    def process_create_order(self, message):
        ...
        if message['type'] == 'PAYMENT':
            ...
            if order_state.state.get_state() == 'CANCELLED PAYMENT':
                publish_msg("sagas_commands", "order.reject", json.dumps(message))
    ...
    def process_cancel_order(self, message):
        order_state = self.__get_order_from_list(message['order_id'])
        if message['type'] == 'DELIVERY':
            order_state.process_cancel_delivery()
            publish_msg("sagas_commands", "delivery.cancel", json.dumps(message))
        if message['type'] == 'PAYMENT':
            order_state.process_cancel_payment()
            publish_msg("sagas_commands", "payment.refund", json.dumps(message))
        if message['type'] == 'ORDER':
            order_state.process_cancel_order()
